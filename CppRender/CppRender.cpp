...
Model GenerateSphere(int divs, const std::array<int, 3> &color)
{
    ...
    for (int d = 0; d < divs; ++d)
    {
        for (int i = 0; i < divs; ++i)
        {
            ...
            // 创建两个三角形
            triangles.push_back(new Triangle(vertexes[i0], vertexes[i1], vertexes[i2], vertexes[i0], vertexes[i1], vertexes[i2], color));
            triangles.push_back(new Triangle(vertexes[i0], vertexes[i3], vertexes[i1], vertexes[i0], vertexes[i3], vertexes[i1], color));
        }
    }
