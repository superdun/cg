...
std::vector<Pixel *> RasterizationRender::DrawShadedTextureTriangle(const std::array<int, 2> p0, const std::array<int, 2> p1, const std::array<int, 2> p2,
																	...
																	const std::array<int, 2> uv0, const std::array<int, 2> uv1, const std::array<int, 2> uv2) const
{
	std::vector<Pixel *> result;
	std::array<int, 2> p0_new = p0;
	std::array<int, 2> p1_new = p1;
...
std::vector<std::vector<double>> RasterizationRender::GetShadowMapBuffer()
{
	ClearDepthBuffer();
	canvas->resetCanvas();
	...
	for (auto pixel : pixels)
	{
		...
		const auto &reciprocalDepth = pixel->GetReciprocalDepth();
		CompareAndSetDepthBuffer(position, reciprocalDepth);
	...
	return std::vector<std::vector<double>>(depthBuffer);
}
...
{
	std::array<std::array<double, 4>, 4> lightCameraMatrix;
	bool shadowFlag = false;
	std::vector<std::vector<double>> shadowMapBuffer;
	double lightIntensity;
	for (const auto light : originalLightList)
	{
		if (light->GetLightType() == LightTypeEnum::DirectionalLight)
		{
			lightCameraMatrix = VectorHelper::MatrixMultiply(
				VectorHelper::Build4DInverseTranslationMatrix(light->GetPosition()),
				VectorHelper::Build4DRotateInverseMatrix(light->GetDirection()));
	...
	if (shadowFlag)
	{
		shadowMapBuffer = GetShadowMapBuffer();
	}
	ClearDepthBuffer();
	canvas->resetCanvas();
	std::vector<const ModelInstance *> clippedInstances;
	...
	for (auto pixel : pixels)
	{
		...
		if (CompareAndSetDepthBuffer(position, pixel->GetReciprocalDepth()))
		{
			if (shadowFlag)
			{
				const std::array<double, 3> matrix_pixel = {position[0], position[1], 1 / pixel->GetReciprocalDepth()};
				const auto matrix_pixelInLightSpace = VectorHelper::Build3DPoint(VectorHelper::VerticeMatrixMultiply(VectorHelper::BuildHomogeneousPoint(matrix_pixel), lightCameraMatrix));
				const auto xPixelInLightSpace = (int)matrix_pixelInLightSpace[0];
				const auto yPixelInLightSpace = (int)matrix_pixelInLightSpace[1];
				if (xPixelInLightSpace < 0 || xPixelInLightSpace >= shadowMapBuffer.size() || yPixelInLightSpace < 0 || yPixelInLightSpace >= shadowMapBuffer[0].size())
				{
					canvas->PutPixel(position[0], position[1], RGB(color[0], color[1], color[2]));
				}
				else
				{
					const auto shadowMapDepth = 1 / shadowMapBuffer[xPixelInLightSpace][yPixelInLightSpace];
					const auto pixelDepth = 1 / pixel->GetReciprocalDepth();
					if (shadowMapDepth < pixelDepth)
					{
						const auto newColor = VectorHelper::ColorVectorScale(color, 1 - lightIntensity);
						canvas->PutPixel(position[0], position[1], RGB(newColor[0], newColor[1], newColor[2]));
					}
					else
					{
						canvas->PutPixel(position[0], position[1], RGB(color[0], color[1], color[2]));
